...
public final class AlgorithmImpl implements Algorithm {
	...
	final var mergedSortedQueues = AlgorithmImpl.mergeSortedQueues(sortedSequence, sortedOutOfOrderQueue);
    ...
    @NonNull
    private static OperationResult<@NonNull IntQueue> mergeSortedQueues(final @NonNull IntQueue sortedSequence,
	    final @NonNull IntQueue remainingSorted) {
	...
	while (!(sortedSequence.empty() || remainingSorted.empty())) {
	    ...
	    final var nextInSequence = sortedSequenceIsNext ? sortedSequence.dequeue() : remainingSorted.dequeue();
